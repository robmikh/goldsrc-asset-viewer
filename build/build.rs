...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for entry in std::fs::read_dir(base_shader_path)? {
        ...
        println!("cargo:rerun-if-changed={}/{}", base_shader_path, entry.path().file_name().unwrap().to_string_lossy());
    }
    ...
    for entry in std::fs::read_dir(base_shader_path)? {
        ...
        if entry.file_type()?.is_file() {
            ...
            if let Some(shader_type) = shader_type {
                ...
                let out_path = format!("{}/{}.spv", generated_shader_path, in_path.file_name().unwrap().to_string_lossy());
